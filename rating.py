...
class Rating(object):
    ...
    def __init__(self,config,trainingSet, testSet):
        ...
        # H = pd.read_csv('./dataset/TCMSP/herb_id_all.txt', sep='\t', header=None, dtype={0: str, 1: str})
        # C = pd.read_csv('./dataset/TCMSP/compound_id_all.txt', sep='\t', header=None, dtype={0: str, 1: str})
        # P = pd.read_csv('./dataset/TCMSP/protein_id_all.txt', sep='\t', header=None, dtype={0: str, 1: str})
        # D = pd.read_csv('./dataset/TCMSP/disease_id_all.txt', sep='\t', header=None, dtype={0: str, 1: str})
        # hlist=H[0].unique()
        # clist=C[0].unique()
        # plist=P[0].unique()
        # dlist=D[0].unique()
